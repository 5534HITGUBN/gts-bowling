    ...
    scoreByFrame(bowlingState, knockedPins){
        ...
        const calculateFrameScore = (updatedFrames, currentFrame) => {
            ...
            updatedFrames[currentFrame].rolls.map((roll, rollIdx) => {
                ...
                console.log(updatedFrames[currentFrame])
                // s1
                ...
                // MIDDLE FRAMES
                else if (currentFrame !== (bowlingState.gameFrameTotal - 1 ) && (currentFrame >= 1) ) {
                    ...
                    else if ((updatedFrames[currentFrame - 1].rolls[0].type === 'strike') && (updatedFrames[currentFrame].rolls[0].type === 'normal' && updatedFrames[currentFrame].rolls[1].type === 'normal')){
                        // do nothing yet if we have two strikes in a row we need another frame's data. 
                        addValues(updatedFrames[currentFrame].rolls, 0, currentFrame);
                    }
                ...
                // LAST FRAME
                else if (currentFrame === (bowlingState.gameFrameTotal - 1)) {
                    let lastFrameTypes = updatedFrames[currentFrame].rolls.map(cur=>cur.type);
                    const checkIfAllStrikes = (el) => {
                        return el === 'strike';
                      }
                    if (lastFrameTypes.every(checkIfAllStrikes)){
    ...
    totalScore(updatedFrames){
        ...
        const targetFrame = () => {
            ...
            if (this.props.bowling.currentFrame > 0) {
                ...
                if (rollType.includes('strike') && updatedFrames[this.props.bowling.currentFrame].rollAttempts === 2) {
    ...
    render() {
        ...
        return (
            <div>
                ...
                <Scoreboard player1={this.props.bowling.player1}  currentFrame={this.props.currentFrame}/>
                {startControls()}
            </div>
        );
    }
